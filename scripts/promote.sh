#!/usr/bin/env bash
# T035: promote the STG-validated Release Set to PROD. Two gates before any PROD change:
#   1. PROD Release Set must be identical to STG's (same image versions — no rebuild).
#   2. STG must currently be healthy.
# Only then apply PROD (via deploy.sh, which is itself health-gated with auto-rollback).
# Usage: promote.sh --client <name>
set -euo pipefail
...
[ -n "$client" ] || { echo "usage: promote.sh --client <name>" >&2; exit 2; }
...
provider="$(provider_of "$stg")"
tf_init "$provider" "$client-stg" >/dev/null
...
echo "gates passed: STG healthy + Release Set matches — promoting to PROD"

exec "$here/deploy.sh" --client "$client" --env prod
