# Private network: VPC with public subnets (ALB) and private subnets (services + DB egress via NAT).
...
resource "aws_security_group" "alb" {
  ...
  vpc_id      = aws_vpc.main.id
  ingress {
    from_port   = local.use_tls ? 443 : 80
    to_port     = local.use_tls ? 443 : 80
    protocol    = "tcp"
...
resource "aws_security_group" "service" {
  ...
  description = "Service-to-service and ALB-to-service traffic (private)"
  vpc_id      = aws_vpc.main.id
  ingress {
