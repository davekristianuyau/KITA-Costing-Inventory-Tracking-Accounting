...
public final class DeductionRuleEngine {
  ...
  /** Employee and employer amounts produced by a rule. */
  public record Amounts(BigDecimal employee, BigDecimal employer) {}
  ...
  public static Amounts evaluate(DeductionRule rule, List<DeductionRuleRow> rows, BigDecimal base) {
    return switch (rule.getComputation()) {
      ...
      case FIXED -> new Amounts(Money.round(nz(rule.getFixedAmount())), Money.zero());
    };
  }
  ...
  private static Amounts table(List<DeductionRuleRow> rows, BigDecimal base) {
    for (DeductionRuleRow r : rows) {
      if (r.contains(base)) {
        return new Amounts(Money.round(nz(r.getEmployeeAmount())), Money.round(nz(r.getEmployerAmount())));
      }
    }
    return new Amounts(Money.zero(), Money.zero());
  }
  ...
  private static Amounts bracket(List<DeductionRuleRow> rows, BigDecimal base) {
    for (DeductionRuleRow r : rows) {
      if (r.contains(base)) {
        ...
        return new Amounts(Money.round(tax), Money.zero());
      }
    }
    return new Amounts(Money.zero(), Money.zero());
  }
  ...
  private static Amounts percent(DeductionRule rule, BigDecimal base) {
    ...
    return new Amounts(employee, employer);
  }
