...
- [X] T001 Create the root `.gitignore` covering Node, Java/Gradle, Docker, and local artifacts
- [X] T002 [P] Create root `README.md` placeholder (title + one-line project description; filled in US4)
- [X] T003 [P] Create top-level directories `frontend/`, `backend/`, `contracts/`, `docs/` with `.gitkeep` placeholders
...
- [X] T004 Define and document the placeholder convention (`.gitkeep` + short `README` per empty dir) in `docs/architecture.md` (stub) so every subsequent directory follows it
...
- [X] T005 [P] [US1] Create `frontend/` subtree with placeholders: `src/`, `tests/` (`.gitkeep` in each)
- [X] T006 [P] [US1] Create `backend/gateway/` subtree with placeholders: `src/main/java/`, `src/main/resources/`, `src/test/java/` (`.gitkeep` in each)
- [X] T007 [P] [US1] Create `backend/reference-service/` subtree with placeholders: `src/main/java/`, `src/main/resources/db/migration/`, `src/test/java/` (`.gitkeep` in each) — the template layout
- [X] T008 [P] [US1] Add a short `README.md` in each service directory (`frontend/`, `backend/gateway/`, `backend/reference-service/`) stating its purpose
- [X] T009 [US1] Add `docs/architecture.md` section listing the tree and each directory's purpose (structure map)
...
- [X] T010 [P] [US2] Create backend `backend/settings.gradle.kts` declaring modules `:gateway` and `:reference-service`
- [X] T011 [P] [US2] Create backend root `backend/build.gradle.kts` skeleton: Java 21 toolchain, Spring Boot/Spring Cloud BOM references, Spotless + Checkstyle plugins (no source)
- [X] T012 [P] [US2] Create per-module build skeletons `backend/gateway/build.gradle.kts` and `backend/reference-service/build.gradle.kts` (declare intended deps; no code)
- [X] T013 [P] [US2] Create `frontend/package.json` (intended deps + `build`/`test`/`lint` script placeholders), `tsconfig.json`, and `vite.config.ts`
- [X] T014 [P] [US2] Create Dockerfile skeletons: `frontend/Dockerfile` (node build → nginx), `backend/gateway/Dockerfile`, `backend/reference-service/Dockerfile`
- [X] T015 [P] [US2] Create `frontend/nginx.conf` skeleton (serve static assets + `/api` proxy, commented as placeholder)
- [X] T016 [US2] Create `docker-compose.yml` wiring postgres + gateway + reference-service + frontend (build contexts → each Dockerfile, env placeholders, network) as scaffolding
- [X] T017 [US2] Create `Makefile` with `build`/`up`/`test`/`lint` target skeletons (documented placeholders, clearly not yet functional)
- [X] T018 [P] [US2] Create `.env.example` documenting intended env vars (DB creds, `GATEWAY_URL`, `REFERENCE_SERVICE_URL`)
...
- [X] T019 [P] [US3] Place `contracts/openapi.yaml` (source-of-truth API contract, to be implemented later)
- [X] T020 [P] [US3] Add frontend quality-gate configs `frontend/.eslintrc` and `frontend/.prettierrc`
- [X] T021 [P] [US3] Add backend quality-gate configs `backend/config/checkstyle.xml` and Spotless config in root build
- [X] T022 [US3] Confirm/annotate `backend/reference-service/` as the copyable template (a `TEMPLATE.md` noting what to copy and where migrations/resources go)
...
- [X] T023 [US4] Flesh out root `README.md`: architecture overview (cloud LB → Nginx → gateway → services → PostgreSQL), repository layout, and scaffolding-only status
- [X] T024 [P] [US4] Write `docs/add-a-service.md` from contracts/service-template.md (how to add a future microservice using the reference-service template)
- [X] T025 [P] [US4] Write `docs/quickstart.md` from specs/002 quickstart (how to verify the scaffold; note nothing runs yet)
...
- [X] T026 Verify the scope guard: repository contains no application source, functional tests, or migration files (FR-013, SC-003)
- [X] T027 [P] Verify every required scaffolding file from data-model.md is present (SC-002) and every empty directory has a placeholder (SC-004)
- [X] T028 [P] Cross-check `docs/architecture.md` against the actual tree for accuracy (SC-005)
