...
All Technical Context items are resolved from the codebase, the constitution, and the two clarify
sessions. **No open NEEDS CLARIFICATION.** D1–D10 are the baseline; D2/D5 revised and D11–D15 added for
the 2026-07-17 clarifications.
...
- **Rationale**: Every service is this shape; 8088 is the next free port (Constitution VI).
...
- **Decision**: Two layers.
  1. **Identity** — the acting `employeeId` comes from gateway header `X-Kita-User` (copied `CallerContext`).
  2. **Validation + roles** — `ActorResolver` calls `HrPort.getEmployee(id)` to confirm the employee is
     **active/not-separated** *and* obtain the **authoritative role set assigned in HR**. Roles are
     **not** taken from `X-Kita-Roles` (self-asserted, untrusted for authz here).
  - **Authorization** — `ActionAuthorizer` (pure) checks HR roles against `authorization_mapping` for
    the requested `BackOfficeAction`. Deny → `ForbiddenException` (403), no downstream call.
- **Rationale**: Clarify 2026-07-17 — HR sets the role at registration; FR-002 resolves roles from HR.
  Stronger than header-trust: a separated/role-revoked employee fails even with a stale token. Pure
  authorizer stays DB-free/unit-testable (Constitution II).
- **Alternatives**: Gateway-token roles — rejected by the clarification. Both-must-agree — rejected as
  unnecessary complexity once HR is authoritative.

## D3. Authorization mapping storage

- **Decision**: Seeded table `authorization_mapping (action, role, kind)` (Flyway `V2`), `kind`
  distinguishing a `MAKER` from a `CHECKER` grant for review-gated actions. Loaded into
  `ActionAuthorizer` at startup.
- **Rationale**: Spec designates it persisted; a table keeps grants versioned/editable via migration
  without redeploy. Still trivial.
...
- **Decision**: Order downstream steps and compensate on failure; no 2PC.
  - **Sales order (US2)**: `SalesOrderWorkflow` drives the lifecycle. DRAFT = create order + add lines +
    ...
    operations `fulfill`, completed) is a separate authorized, attributed transition. Step failure →
    **cancel** the operations order (compensation). Operations order is the system of record; the review
    position is transient (D14) and rebuildable.
...
- **Rationale**: Strongest atomicity stays where writes are colocated (operations build; procurement
  ...
  order). No 2PC (spec Assumptions; Constitution VI).
...
- **Decision**: `HrPort.getEmployee(id)` returns `EmployeeView { UUID id; boolean active; Set<String> roles }`.
  workflow-service maps role strings to its `Role` tokens. Used for the whole request (one HR call per
  action; no long-lived cache — SMB scale makes per-action lookup fine).
- **Rationale**: Clarify — roles assigned in HR at registration (D2). Single source of truth.
- **Dependency/risk**: hr-service must expose an employee's assigned back-office role tokens. If absent,
  it's a small hr-service addition (or seed); the **fake** `HrPort` supplies roles so workflow-service is
  testable in isolation regardless. Flagged in plan Dependencies.
- **Alternatives**: Derive roles from job title/department — rejected (indirect, brittle). Cache roles in
  workflow-service — rejected (duplicates HR master, violates FR-017).
...
- **Decision**: For review-gated actions (goods-receipt confirmation; sales-order payment confirmation and
  ...
  come from a **different `employeeId`** holding the **checker** role (`authorization_mapping` `kind=CHECKER`).
...
- **Alternatives**: Same-role-different-person only — rejected; the clarification says the roles are
  distinct (maker role ≠ checker role), implying different persons.
...
- **Rationale**: From the clarification (draft until payment confirmed by manager/cashier; release after
  packed check; completed once handed to customer). Reservation lives durably in operations from DRAFT;
  the review position is transient (D14).
- **Alternatives**: One-shot create+confirm — rejected by the clarification. Adding all 4 statuses to
  operations-service — rejected for now (avoids changing implemented spec 003; gates without a distinct
  operations status are carried transiently + logged). Revisit if a durable operations status is wanted.
...
- **Decision**: `PendingReviewStore` port with a default **`InMemoryPendingReviewStore`** (a
  ...
  **removed**. The store is **derived/working state** — reconstructable from the domain services (records
  in the relevant status) and the activity log — so its loss causes at most re-work, never a lost or
  half-applied domain effect.
- **Rationale**: The user's model — goes on a queue/cache first; once state updates the next downstream
  service receives it; once complete, clear it; final update in the DB. Keeps the two durable stores +
  FR-017 intact (transient ≠ persisted master). A plain in-memory map is the simplest thing at SMB scale
  (Constitution VI); the **port lets a Redis/queue adapter replace it later** with no caller change, like
  the http/fake split.
...
- **Alternatives**: A third durable `pending_approval` table — rejected (clarification chose transient; a
  table would edge toward owning workflow state). Durable queue infra now — rejected (YAGNI; interface
...
- **Rationale**: The clarification chose auto-retry-then-report. Idempotency keys make retries safe (no
  duplicate side effect — SC-010). A tiny helper avoids spring-retry (YAGNI); pure and unit-testable.
- **Alternatives**: spring-retry dependency — rejected (a bounded loop suffices). No retry / report-only —
  rejected by the clarification.

<!-- token-budget: compacted (level=medium) on 2026-07-17T10:14:44Z; original at research.full.md -->
